...
* **DOCKER_COMMIT_IMAGE** (default: false): If set to true, the docker images which contains the build and test will be saved in the outer-layer docker which runs the script.  The commited image is then be available to be pushed to a docker registry.  This behavior enables a building Docker images with CI/CD approach. If set to false, or unset the container will not be commited and is removed.
* **DOCKER_COMMIT_IMAGE_MSG** (default: not set): used to specify a commit during the docker commit command which is triggered by setting DOCKER_COMMIT_IMAGE=true. If unset and if DOCKER_COMMIT_IMAGE=true then the commit message will be empty.
* **DOCKER_COMMIT_IMAGE_NAME** (default: not set): used to specify an image name during the docker commit command which is triggered by setting DOCKER_COMMIT_IMAGE=true. If unset and if DOCKER_COMMIT_IMAGE=true then container id will be used as the image name.
